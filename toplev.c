...
/* GIL: value of the -funroll-max nnn switch, default is 20 */
extern int MAX_UNROLLED_INSNS = 20;
...
/* Gil: turn exceptions off by default */
...
/* GIL: 25.11.97 - don't remove directory name... again, what fool did this? */  
  char* na = input_name;
#if 0
     ...
     char **envp;
{
  ...
  for (i = 1; i < argc; i++)
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
	{
	  ...
	  /* GIL: make "-funroll-max=nnn" a legit option */
	  else if (!strncmp (str, "funroll-max=", 12))
	      MAX_UNROLLED_INSNS = atoi (str+12);
	  /* End GIL */
