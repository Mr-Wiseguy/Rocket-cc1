...
/* Nonzero means process each loaded file for Kanji fake-escape sequences */
/* Gil 8.7.97 */
static int kanji = 0;
...
/* Gil 8.7.97 - patch to scan buffer for kanji escapes */
...
/* Reallocate to allow for 'slashcount' extra characters */

  newbuf = (char*) xmalloc (fp->length + slashcount + 2);
  op = newbuf;
...
/* GIL: recognise -kanji option */
      case 'k':
      if (!strcmp (argv[i], "-kanji")) {
	kanji = 1;
      }
      break;
/* End GIL */
  ...
  if (print_deps) {
    ...
    if (deps_target) {
      ...
      char *p, *q;
      int len;
/* GIL 23.2.98 - don't remove directory!
...
*/
  ...
  /* Gil 8.7.97 patch for Kanji-detection pass */
  if (kanji)
    kanji_preprocess (fp);
     ...
     struct file_name_list *dirptr;
{
  ...
  /* Gil 8.7.97 patch for Kanji preprocess pass */
